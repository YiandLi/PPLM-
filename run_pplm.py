...
def perturb_past(
        ...
        model,
        last,  # 最后一个 token id
        unpert_past=None,  # 原始序列的 [past_key_values]
        unpert_logits=None,
    ...
    past = [torch.vstack(i) for i in past]
    _, _, curr_length, _ = past[0].shape
    
    if curr_length > window_length and window_length > 0:  # 论文中的 window size
    ...
    for i in range(num_iterations):  # TODO：迭代数次
        print("Iteration ", i + 1)
        curr_perturbation = [torch.from_numpy(p_).requires_grad_(True).to(device=device) for p_ in
                             grad_accumulator]  # TODO：将 grad_accumulator 设置为  梯度
        # make sure p_.grad is not None
        for p_ in curr_perturbation:
            p_.retain_grad()
        
        # Compute hidden using perturbed past
        perturbed_past = list(map(add, past, curr_perturbation))  # TODO  对应公式，确实是相加
        _, _, _, curr_length, _ = curr_perturbation[0].shape
        lm_output = model(last, past_key_values=perturbed_past)  # TODO 使用更新过的 perturbed_past 得到输出
        all_logits, all_hidden = lm_output["logits"], lm_output[
            "hidden_states"]  # TODO 得到每个token的点积相似度 和  hidden  state
        hidden = all_hidden[-1]  # TODO： 最后一层的hidden state
        new_accumulated_hidden = accumulated_hidden + torch.sum(hidden, dim=1).detach()  # TODO  求 Sum
        
        # TODO: Check the layer-norm consistency of this with trained discriminator (Sumanth)
        #  这里是为了 KL 散度用的，求最后一个token 的点积相似度 -> [batch_size, 1, vocab_size]
        logits = all_logits[:, -1, :]  # TODO 最后一个token 的点积相似度
        probs = nn.functional.softmax(logits, dim=-1)  # TODO：最后一个 token的概率  [batch_size, 1, vocab_size]
        ...
        if loss_type == 2 or loss_type == 3:  # 使用 classifer
            ce_loss = nn.CrossEntropyLoss()
            # TODO why we need to do this assignment and not just using unpert_past? (Sumanth)
            curr_unpert_past = unpert_past
            curr_probs = torch.unsqueeze(probs, dim=1)  # [batch_size, vocab_size] 表示最后一个 token
            
            wte = model.resize_token_embeddings()
            for _ in range(horizon_length): # TODO：也可以选择让模型连续预测 多个 token，有点意思
                inputs_embeds = torch.matmul(curr_probs,
                                             wte.weight.data)  # TODO：得到 SOFT emb [batch_size, hidden_size]
                ...
                curr_logits = curr_all_logits[:, -1, :]  # TODO：新的  last——token [batch_size, 1, vocab_size]
                curr_probs = nn.functional.softmax(curr_logits, dim=-1)
                curr_probs = torch.unsqueeze(curr_probs, dim=1)
                curr_hidden = curr_all_hidden[-1]  # 最后一层
            ...
            loss += discrim_loss  # 还要考虑 kl loss
...
def full_text_generation(
    ...
    unpert_gen_tok_text, _, _ = generate_text_pplm(  # 最重要的函数
        model=model,
        tokenizer=tokenizer,
        context=context,
        device=device,
...
def generate_text_pplm(
    ...
    if context:
        ...
        while len(context_t.shape) < 2:  # 增加 batch_size 维度
    ...
    for i in trange(length, ascii=True):
        ...
        if past is None and output_so_far is not None:  # 没有提供 past 序列，即 past_key_values
            ...
            if output_so_far.shape[1] > 1:
                past = model(output_so_far[:, :-1])[
                    "past_key_values"]  # 选出最后一个token之前的序列，返回 odict_keys(['logits', 'past_key_values', 'hidden_states'])
        ...
        unpert_last_hidden = unpert_all_hidden[-1]  # 原始序列，对应输出的最后层 hidden state
        ...
        if not perturb or num_iterations == 0:
            ...
            accumulated_hidden = unpert_last_hidden[:, :-1, :]  # batch_size, seq_len-1, hidden_state
            accumulated_hidden = torch.sum(accumulated_hidden,
                                           dim=1)  # 将最后一层的所有 hidden state 求和： batch_size, 1, hidden_state
        ...
        for token_idx in set(output_so_far[0].tolist()):
            if pert_logits[0, token_idx] < 0:  # 和前面的 token 相比，如果 相似度小于 0
                ...
                pert_logits[0, token_idx] /= repetition_penalty  # 不想预测出来重复的 token
        ...
        if perturb:
            ...
            unpert_probs = nn.functional.softmax(unpert_logits[:, -1, :], dim=-1)
            # TODO：然后就是融合修改前后的生成token的输出概率，
            #  修改前的为 `unpert_probs`，修改后的为`pert_probs`，这里使用超参数 `gm_scale`
