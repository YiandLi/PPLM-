...
class Discriminator(nn.Module):
    ...
    def avg_representation(self, x):
        mask = x.ne(0).unsqueeze(2).repeat(1, 1, self.embed_size).float().to(self.device).detach()  # batch_size, max_seq_len, hidden_state
        hidden = self.encoder.transformer(x)["last_hidden_state"]  # batch_size, max_seq_len, hidden_state
        masked_hidden = hidden * mask
        avg_hidden = torch.sum(masked_hidden, dim=1) / (torch.sum(mask, dim=1).detach() + EPSILON)  # batch_size, hidden_state 每个句子中所有 token 的 avg
        return avg_hidden
...
def train_discriminator(
    ...
    if dataset == "SST":
        ...
        test_x = []
        test_y = []
        # for i in trange(len(test_data)[:20], ascii=True):
        for i in trange(20, ascii=True):
